...
def write_csv_safely(frame: pd.DataFrame, target: Path) -> Path:
    """Р—Р°РїРёСЃС‹РІР°РµС‚ CSV Р±РµР·РѕРїР°СЃРЅРѕ РґР»СЏ Windows.

    Р¦РµР»РµРІРѕР№ С„Р°Р№Р» РјРѕР¶РµС‚ Р±С‹С‚СЊ РѕС‚РєСЂС‹С‚ РІ Excel, Р±СЂР°СѓР·РµСЂРµ РёР»Рё РѕРєРЅРµ РїСЂРµРґРїСЂРѕСЃРјРѕС‚СЂР°.
    РџРѕСЌС‚РѕРјСѓ СЃРЅР°С‡Р°Р»Р° РїРёС€РµРј РІРѕ РІСЂРµРјРµРЅРЅС‹Р№ С„Р°Р№Р», Р·Р°С‚РµРј Р°С‚РѕРјР°СЂРЅРѕ Р·Р°РјРµРЅСЏРµРј target.
    Р•СЃР»Рё target Р·Р°Р±Р»РѕРєРёСЂРѕРІР°РЅ, СЃРѕС…СЂР°РЅСЏРµРј СЂСЏРґРѕРј РѕС‚РґРµР»СЊРЅС‹Р№ *_FOR_UPLOAD.csv.
    """
    ...
    try:
        ...
        print(
            f"[FILE-LOCK] {target.name} СЃРµР№С‡Р°СЃ Р·Р°РЅСЏС‚ Windows. "
            f"РўРѕС‡РЅС‹Р№ С„Р°Р№Р» СЃРѕС…СЂР°РЅС‘РЅ РєР°Рє {fallback.name}.",
            flush=True,
        )
        return fallback
...
def load_json_report(name: str) -> dict:
    """Читает служебный JSON из корня проекта или из аккуратной папки reports."""

    for path in [Path(name), Path("reports") / name]:
        if path.exists():
    ...
    for row_id, row in frame.reset_index(drop=True).iterrows():
        for column_id, column in enumerate(TEXT_COLUMNS):
            ...
            for index, (match, amount) in enumerate(parsed):
                ...
                money_pattern = r"в‚Ѕ|СЂСѓР±|СЂ\.?\s*(?:/|$)|С‚С‹СЃ|\bk\b|\$|usd|eur|РµРІСЂРѕ"
                salary_pattern = r"Р·Р°СЂРїР»Р°С‚|РѕРєР»Р°Рґ|Р·/?Рї\b|РґРѕС…РѕРґ|РѕРїР»Р°С‚|РІРѕР·РЅР°РіСЂР°Р¶|salary"
                bad_pattern = (
                    r"РјР±РёС‚|РіР±РёС‚|С‡Р°СЃ|РґРЅ(?:РµР№|СЏ)|Р»РµС‚|РіРѕРґ|%|РїСЂРѕС†РµРЅС‚|СЃРѕС‚СЂСѓРґРЅРёРє|"
                    r"РєРІ\.?\s*Рј|РјРµС‚СЂ|Р·Р°РєР°Р·|СЃРјРµРЅ|РїСЂРѕРµРєС‚|СѓСЂРѕРє"
                )
                range_pattern = r"^\s*(?:[-вЂ“вЂ”]|РґРѕ\b|РїРѕ\b)"
                rows.append(
                    {
                        "row_id": row_id,
                        "amount": amount,
                        ...
                        "context_preceded_from": float(bool(re.search(r"\bРѕС‚\s*$", left_near, re.I))),
                        "context_preceded_to": float(bool(re.search(r"\bРґРѕ\s*$", left_near, re.I))),
                        "context_followed_period": float(bool(re.search(r"^\s*(?:СЂСѓР±\.?\s*)?/(?:РјРµСЃ|РјРµСЃСЏС†|month)|РІ\s+РјРµСЃСЏС†", right_near, re.I))),
...
# РџСЂРё РїРѕР»РЅРѕРј РїРµСЂРµРѕР±СѓС‡РµРЅРёРё РѕРґРёРЅ РёР· СЂР°РЅРЅРёС… test-СЃР°Р±РјРёС‚РѕРІ РјРѕР¶РµС‚ РїРѕС‚РµСЂСЏС‚СЊ
# РєР°Р»РёР±СЂРѕРІРєСѓ, С…РѕС‚СЏ РµРіРѕ local OOF РѕСЃС‚Р°С‘С‚СЃСЏ РЅРѕСЂРјР°Р»СЊРЅС‹Рј. Р­С‚Рѕ РѕСЃРѕР±РµРЅРЅРѕ РѕРїР°СЃРЅРѕ
# РґР»СЏ stacking: Ridge РІРёРґРёС‚ С…РѕСЂРѕС€РёР№ local final_boost Рё РґР°С‘С‚ РµРјСѓ Р±РѕР»СЊС€РѕР№ РІРµСЃ,
# Р° РЅР° test РїРѕР»СѓС‡Р°РµС‚ СЃРґРІРёРЅСѓС‚С‹Р№ СЃРёРіРЅР°Р». РџРѕСЌС‚РѕРјСѓ РїСЂРѕРІРµСЂСЏРµРј final_boost РЅР°
# СЂР°СЃРїСЂРµРґРµР»РёС‚РµР»СЊРЅС‹Р№ РґСЂРµР№С„ РѕС‚РЅРѕСЃРёС‚РµР»СЊРЅРѕ РѕСЃС‚Р°Р»СЊРЅС‹С… СЃС‚Р°Р±РёР»СЊРЅС‹С… РјРѕРґРµР»РµР№ Рё, РµСЃР»Рё
# РЅР°РґРѕ, РІРѕСЃСЃС‚Р°РЅР°РІР»РёРІР°РµРј test final_boost С‡РµСЂРµР· surrogate, РѕР±СѓС‡РµРЅРЅС‹Р№ РЅР° OOF.
...
local_best = load_json_report("oof_candidate_reranker_086_results.json")
report = {
...
(Path("reports") / REPORT.name).write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")
